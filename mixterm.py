...
class MixtureOfSHOsTerm(terms.SHOTerm):
    ...
    def get_complex_coefficients(self, params):
        ...
        #print "c1, c2", c1, c2
        #print "log_S2, log_qQ2, log_omega2", log_S2, log_Q2, log_omega2
        #print "comp", log_a, log_omega1, log_Q1, mix, log_omega2, log_Q2
        #print "further", np.log(4*np.pi*Q), np.log(P),  0.5*np.log(4.0*Q*Q-1.0), Q
        return [np.array([a, b]) for a, b in zip(c1, c2)]

    def log_prior(self):
        lp = super(MixtureOfSHOsTerm, self).log_prior()
        #print "lp", lp, self.mix_par
        if not np.isfinite(lp):
            return -np.inf
        mix = 1.0 / (1.0 + np.exp(-self.mix_par))
        #print "mix", mix, self.mix_par, (1.0 + np.exp(-self.mix_par))
